...
const FROM_EMAIL = process.env.EMAIL_FROM || "ShopEase <onboarding@resend.dev>";

// Send email via Resend HTTP API
const sendEmail = async ({ to, subject, html, text }) => {
  console.log(`📧 Sending via Resend to: ${to} | Subject: ${subject}`);
...
const getResetPasswordEmailTemplate = (resetUrl, userEmail = '') => {
  ...
  .card{background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 25px 50px rgba(0,0,0,.3);}
  ...
  .btn{display:inline-block;padding:16px 40px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;text-decoration:none;border-radius:50px;font-weight:bold;font-size:1.1rem;margin:20px 0;}
...
const getPasswordChangedConfirmationTemplate = (userEmail) => {
  ...
  .container{max-width:600px;margin:40px auto;padding:20px;}
  .card{background:#fff;border-radius:20px;overflow:hidden;text-align:center;padding:40px;}
